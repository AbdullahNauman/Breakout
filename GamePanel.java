...
public class GamePanel extends JPanel implements ActionListener, KeyListener
{
  ...
  private Ball ball;
  private boolean hasLost, hasWon, hasReleased, redraw;// Both initialized as false in
  ...
  public void paintComponent(Graphics g)
  {
    ...
    if (hasWon)
    {
      ...
      drawText(g, (int) (leftBorder.getWidth() + 20),
          (int) panelSize.getHeight() / 2 + 30, "Nice job! Press space to play again",
          30f);
    }
    ...
    for (int x = 0; x < blocksToBreak.length; x++)
    {
      for (int y = 0; y < blocksToBreak[x].length; y++)
      {
            ...
            && blocksToBreak[x][y].getHardness() > 0)
        {
          ...
          {
            ball.setObjVelX(ball.getObjVelX() * -1);
            if (ball.getCenterX() > blocksToBreak[x][y].getCenterX())// Moves ball out of intersection
          ...
          {
            ball.setObjVelY(ball.getObjVelY() * -1);
            if (ball.getCenterY() > blocksToBreak[x][y].getCenterY())// Moves ball out of intersection
