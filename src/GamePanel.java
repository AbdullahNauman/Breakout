...
public class GamePanel extends JPanel implements ActionListener, KeyListener
{
  ...
  private Ball ball;
  private boolean hasLost, hasWon, hasReleased, redraw;// Both initialized as
                                                       // false in
  // constructor. hasLost changes to
  // true when ball moves below paddle.
  // hasReleased changes when ball is
  // released
  private Rectangle topBorder, leftBorder, rightBorder;
  private Block blocksToBreak[][];
  private final int sideBorderScale = 100, topBorderScale = 60,
  ...
  public void paintComponent(Graphics g)
  {
    ...
    if (hasLost)
    {
      ...
      scoring.paintComponent(g, (int) (leftBorder.getWidth() + 20),
          (int) panelSize.getHeight() / 2, "Final Score: ");
    ...
    if (!hasReleased)
    {
      ...
      drawText(g, (int) (leftBorder.getWidth() + 20),
          (int) panelSize.getHeight() / 2 + 30, "Press space to release ball",
          40f);
    }
  }
  ...
  {
    if (hasLost || hasWon) // Checks loss condition and stops timer if true.
                           // Resets paddle width.
    {
            ...
            && blocksToBreak[x][y].getHardness() > 0)
        {
          ...
          if (intersection.getHeight() > intersection.getWidth())
          {
            ...
            if (ball.getCenterX() > blocksToBreak[x][y].getCenterX())// Moves
                                                                     ...
                                                                     // of
                                                                     // intersection
              ball.setLocation((int) blocksToBreak[x][y].getMaxX() + 1,
                  (int) ball.getY());
            else
              ball.setLocation((int) blocksToBreak[x][y].getMinX() - 1,
                  (int) ball.getY());
          }
          else
          {
            ...
            if (ball.getCenterY() > blocksToBreak[x][y].getCenterY())// Moves
                                                                     ...
                                                                     // of
                                                                     // intersection
              ball.setLocation((int) ball.getX(),
                  (int) blocksToBreak[x][y].getMaxY() + 1);
            else
              ball.setLocation((int) ball.getX(),
                  (int) blocksToBreak[x][y].getMinY() - 1);
          }
    ...
    for (int x = 0; x < blocksToBreak.length; x++)
    {
      for (int y = 0; y < blocksToBreak[x].length; y++)
      {
        ...
        blocksToBreak[x][y].setLocation(
            ...
            yPos);// Addition to xPos in argument 1 is to
  ...
  public void keyPressed(KeyEvent e)
  {
    if (e.getKeyCode() == KeyEvent.VK_SPACE && hasLost)
    {
      hasLost = false;
      initializeBlocks();
      ball.setLocation((int) paddle.getX(),
          (int) paddle.getY() - (int) ball.getHeight());
      scoring.setScore(0);
      timer.start();
    }
