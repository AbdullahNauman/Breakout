...
public class GamePanel extends JPanel implements ActionListener {
	...
	private final int sideBorderScale = 40, topBorderScale = 30, blockPosScale = 5, blockYSpaceScale = 22,
			textShiftX = -60, textShiftY = 12;// Scaling factors for border
												// edges, block positioning,
	...
	public GamePanel() {
		...
		for (int x = 0; x < blocksToBreak.length; x++) {
			for (int y = 0; y < blocksToBreak[x].length; y++) {
				int hardness = (blocksToBreak.length - x + 1) / 2;// Calculates
																	// hardness
																	// level
																	// based on
																	// row
	...
	public void paintComponent(Graphics g) {
		...
		paddle.paintComponent(g);// Draws paddle component of appropriate size
									// and
									// in the correct position based on data
									// stored in
									// Paddle class
	...
	private void collisionDetection(Dimension panelSize) {
		...
		if (paddle.intersects(leftBorder)) // Prevents paddle from moving
											// off-screen
		...
		for (int x = 0; x < blocksToBreak.length; x++) {
			for (int y = 0; y < blocksToBreak[x].length; y++) {
				if ((blocksToBreak[x][y].intersects(ball) && blocksToBreak[x][y].getHardness() > 0)
						&& (blocksToBreak[x + 1][y].intersects(ball) && blocksToBreak[x][y].getHardness() > 0)) {
					blocksToBreak[x][y].breakBlock();
					blocksToBreak[x + 1][y].breakBlock();
		...
		/*	else if (blocksToBreak[x][y].intersects(ball) && blocksToBreak[x][y].getHardness() > 0) {
				blocksToBreak[x][y].breakBlock();
				if (blocksToBreak[x][y].getX() >= ball.getX() + ball.getDiameter()
						|| blocksToBreak[x][y].getX() + Block.width <= ball.getX()) {
					ball.setObjVelX(ball.getObjVelX() * -1);
	...
	*/ 
	...
	private void setBlockPos(Dimension panelSize) {
		...
		double accuratePadding = (playingWidth - blocksToBreak[0].length * Block.width)
				/ ((double) (blocksToBreak[0].length - 1));// Double version of
															// padding
															// variable so that
															// error
															// from integer
		...
		for (int x = 0; x < blocksToBreak.length; x++) {
			for (int y = 0; y < blocksToBreak[x].length; y++) {
				...
				blocksToBreak[x][y].setLocation(xPos + (int) (accuratePadding * (blocksToBreak[0].length - 1)
						- padding * (blocksToBreak[0].length - 1)) / 2, yPos);// Addition
																				// to
																				// xPos
																				// in
																				// argument
